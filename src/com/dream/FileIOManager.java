...
//        try {
    ...
    private void saveStateToStream(DataOutputStream outStream, ApparatModel model) throws IOException {
        ...
        for (int i = 0; i < model.getCh1DataStream().size(); i++) {
            ...
            outStream.writeInt(model.getCh2DataStream().get(i));
            if((i % accDivider) == 0) {
                outStream.writeInt(model.getAcc1DataStream().get(i / accDivider));
                outStream.writeInt(model.getAcc2DataStream().get(i / accDivider));
                outStream.writeInt(model.getAcc3DataStream().get(i / accDivider));
            }
        }
    }
